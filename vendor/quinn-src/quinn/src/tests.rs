...
async fn zero_rtt() {
    ...
    const MSG1: &[u8] = b"one";
    const EXPORTER_LABEL: &[u8] = b"tuic-0rtt-test";
    const EXPORTER_CONTEXT: &[u8] = b"password";
    let (early_exporter_tx, early_exporter_rx) = tokio::sync::oneshot::channel();
    let endpoint2 = endpoint.clone();
    tokio::spawn(async move {
        let mut early_exporter_tx = Some(early_exporter_tx);
        for attempt in 0..2 {
            ...
            if attempt == 1 {
                ...
                connection
                    .export_early_keying_material(&mut exported, EXPORTER_LABEL, EXPORTER_CONTEXT)
                    .expect("server early exporter");
                early_exporter_tx.take().unwrap().send(exported).unwrap();
            }
    ...
    let mut client_exported = [0; 32];
    connection
        .export_early_keying_material(&mut client_exported, EXPORTER_LABEL, EXPORTER_CONTEXT)
        .expect("client early exporter");
    assert_eq!(client_exported, early_exporter_rx.await.unwrap());
