...
pub trait Session: Send + Sync + 'static {
    ...
    /// Fill `output` using the TLS 1.3 early exporter secret.
    ///
    /// This is available before handshake completion only when the session
    /// has 0-RTT key material.
    fn export_early_keying_material(
        &self,
        _output: &mut [u8],
        _label: &[u8],
        _context: &[u8],
