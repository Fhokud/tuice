...
mod connection {
    ...
    impl Connection {
        ...
        /// Derives key material from the TLS 1.3 early exporter secret.
        ///
        /// This is available only when 0-RTT keys were derived for this
        /// connection. It does not wait for the handshake to complete.
        pub fn export_early_keying_material<T: AsMut<[u8]>>(
            &self,
            mut output: T,
            label: &[u8],
            context: Option<&[u8]>,
            ...
            if output.as_mut().is_empty() {
                return Err(Error::General(
                    "export_early_keying_material with zero-length output".into(),
                ));
            }
            let exporter = match self {
                Self::Client(conn) => conn.core.common_state.quic.early_exporter.as_ref(),
                Self::Server(conn) => conn.core.common_state.quic.early_exporter.as_ref(),
            }
            .ok_or(Error::HandshakeNotComplete)?;
            exporter.export(output.as_mut(), label, context)?;
            Ok(output)
        }
    }
...
pub(crate) struct Quic {
    ...
    pub(crate) early_exporter: Option<crate::tls13::key_schedule::EarlyExporter>,
